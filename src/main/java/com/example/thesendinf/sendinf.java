package com.example.thesendinf;
...
public class sendinf {
    ...
    //需要考虑如何定义消息类型,用前256个字节定义,第一位数据包类型,第二位文件名,第三位文件分块,第四位当前块数.第五位,填充0
    {
        ...
        for(int i=0;i<times;i++)
        {
            if(i<times-1)//前面的文件块
            {
                StringBuilder packetinf = new StringBuilder(showthefileinf.getfileinf(String.valueOf(thefile), useinf,flag) + times + "//" + i + "//");
//                System.out.println(packetinf);
            ...
            if(i>=times-1)//最后一块内容
            {
                StringBuilder packetinf = new StringBuilder(showthefileinf.getfileinf(String.valueOf(thefile), useinf,flag) + times + "//" + (times - 1) + "//");
    ...
    public static void sendthestr(SocketAddress address, DatagramSocket client, String str, StringBuilder head) {
        if(head.toString().getBytes(StandardCharsets.UTF_8).length<HEADLENGTH)
        {
            ...
            head.append("*".repeat(Math.max(0, templength)));
        }
